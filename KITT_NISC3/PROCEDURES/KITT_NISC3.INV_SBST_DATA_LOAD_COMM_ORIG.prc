DROP PROCEDURE INV_SBST_DATA_LOAD_COMM_ORIG;

CREATE OR REPLACE PROCEDURE            "INV_SBST_DATA_LOAD_COMM_ORIG"
AS
  invoice_no NUMBER := 56;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    IF (c_nat_cont_comments IS NOT NULL) THEN
      comments_array        := apex_util.string_to_table(c_nat_cont_comments, '|');
      FOR i IN 1..comments_array.count
      LOOP
        INSERT
        INTO INV_SUBST_COMM_FROM_NAT VALUES
          (
            n_invoice_no,
            n_invoice_item_no,
            comments_array(i),
  ...
  END LOOP;
  COMMIT;
END INV_SBST_DATA_LOAD_COMM_ORIG;
/
