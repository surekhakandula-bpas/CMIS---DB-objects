DROP PROCEDURE TOOL_XFER_NON_FEE_TO_FEE;

CREATE OR REPLACE PROCEDURE TOOL_XFER_NON_FEE_TO_FEE
...
BEGIN
  ...
  UPDATE FEE
     SET N_FEE_AMOUNT = N_FEE_AMOUNT + Amount
       , T_COMMENT = T_COMMENT || '  - $' || Amount || ' for Non-Fee->Fee Transfer by ' || UserName
   WHERE N_DELPHI_OBLIGATION_ID = DelphiObligation;

  LogMsg := '$' || Amount || ' was transfered from NON-FEE to FEE by user ' || UserName || ' on Obligation# ' || DelphiObligation;
  ...
  AuditMsg := '<TRANSACTION><User_Id>'|| UserName ||'</User_Id><Action>TransferNonFeeToFee</Action><Object_Type>Obligation</Object_Type>' ||
                  '<Object_Id>' || DelphiObligation || '</Object_Id><Amount>' || Amount || '</Amount></TRANSACTION>';
  INSERT INTO KITT_AUDIT VALUES
      ( UserId, SYSDATE, 'Non-Fee Transfer to Fee', XMLType(AuditMsg) );
END TOOL_XFER_NON_FEE_TO_FEE;
/
