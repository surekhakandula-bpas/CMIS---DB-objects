DROP PROCEDURE SP_KITT_DATA_REFRESH;

CREATE OR REPLACE PROCEDURE            SP_KITT_DATA_REFRESH
(
...
/*
---------------------------------------------------------------------------------------------------
  Object Name:  SP_KITT_DATA_REFRESH
  Author:       J Feld
...
IS
  ...
  v_PROCEDURE_NM  VARCHAR2(100) := 'SP_KITT_DATA_REFRESH';
...
--   select 'insert /*+ APPEND */ into '||p_SCHEMA||'.'||table_name||'@'||v_DB_LINK||' select * from '||p_SCHEMA||'.'||table_name||';'
   select 'insert /*+ APPEND */ into KITT_NISC3.'||table_name||'@KITT_NISC3_'||p_DATABASE||' select * from KITT_NISC3.'||table_name||';'
       from dba_tables
