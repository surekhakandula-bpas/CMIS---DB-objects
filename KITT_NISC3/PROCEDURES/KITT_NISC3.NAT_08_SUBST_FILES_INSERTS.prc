DROP PROCEDURE NAT_08_SUBST_FILES_INSERTS;

CREATE OR REPLACE PROCEDURE            "NAT_08_SUBST_FILES_INSERTS" (
   InvoiceID    NUMBER)
AS
   invoice_no                NUMBER;
...
BEGIN
   ...
   FOR cur_filenames
      IN (SELECT n.invoice_number,
           ...
           WHERE     n.invoice_number = invoice_no
                 AND n.external_id = i.n_mis_invoice_item_id   --- for testing
   ...
   LOOP
      ...
      IF (nat_display_file_names IS NOT NULL)
      THEN
         v_display_names_array :=
            APEX_UTIL.string_to_table (nat_display_file_names, '|');
         v_physical_names_array :=
            APEX_UTIL.string_to_table (nat_physical_file_names, '|');
   ...
   COMMIT;
END NAT_08_SUBST_FILES_INSERTS;
/
