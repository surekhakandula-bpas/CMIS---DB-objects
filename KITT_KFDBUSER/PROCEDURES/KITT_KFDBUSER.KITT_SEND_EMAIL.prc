DROP PROCEDURE KITT_SEND_EMAIL;

CREATE OR REPLACE PROCEDURE               KITT_SEND_EMAIL (
                                       sender     IN VARCHAR2,
...
BEGIN
   ...
   mesg :=
      ...
      || crlf
      || 'From:  KITT Financial Module AutoGenerator'
      || crlf
      || 'Subject: '
      || subject
      || crlf
      ...
      || crlf
      ||'The KITT Financial Module interfaces have updated KITT with Delphi data as of '||sysdate||' and the following records are brought to your attention:'
